...
class GW2_Password_Reset {
    ...
    public static function init() {
        add_action('wp_ajax_nopriv_gw2gl_request_password_reset', [__CLASS__, 'handle_request_reset']);
        add_action('wp_ajax_nopriv_gw2gl_redeem_magic_link', [__CLASS__, 'handle_redeem_magic_link']);
        add_shortcode('gw2gl_password_reset', [__CLASS__, 'render_reset_form']);
    }
    ...
    public static function handle_request_reset() {
        ...
        wp_mail($email, __('Your GW2 Guild Login Password Reset Link', 'gw2-guild-login'),
    ...
    public static function handle_redeem_magic_link() {
        ...
        $saved_token = get_user_meta($user_id, self::RESET_META_KEY, true);
        $expiry = (int) get_user_meta($user_id, self::RESET_EXPIRY_META_KEY, true);
     ...
     */
    public static function render_reset_form($atts = []) {
        ob_start();
        ?>
