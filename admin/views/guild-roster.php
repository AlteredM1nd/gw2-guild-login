...
$members = []; $available_ranks = [];
if (!is_wp_error($data) && is_array($data)) {
    $members = is_array($data['members'] ?? []) ? $data['members'] : [];
    $available_ranks = is_array($data['ranks'] ?? []) ? $data['ranks'] : [];
}

// Handle rank filter
$filter_rank = isset($_GET['filter_rank']) ? sanitize_text_field($_GET['filter_rank']) : '';
if ($filter_rank !== '') {
    $members = array_filter($members, function($m) use ($filter_rank) {
        ...
        <tbody>
            ...
            <?php else : ?>
                <?php foreach ($members as $member) : ?>
                    ...
                    $users = get_users([ 'meta_key' => 'gw2_account_name', 'meta_value' => $name, 'number' => 1 ]);
                    $last_login = '';
