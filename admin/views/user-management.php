...
<div class="wrap gw2-admin-user-management">
	...
	<form method="post">
		...
		if ( ! empty( $_GET['filter_role'] ) ) {
			$args['role'] = isset( $_GET['filter_role'] ) ? sanitize_key( wp_unslash( $_GET['filter_role'] ) ) : '';
		}
		...
		<table class="wp-list-table widefat fixed striped">
			...
			?>
			<p><?php esc_html_e( 'No guild members found.', 'gw2-guild-login' ); ?></p><?php endif; ?>
	</form>
</div>
...
<form method="post">
	...
	<table class="form-table">
		...
		foreach ( $roles as $slug => $info ) :
			?>
			<option value="<?php echo esc_attr( $slug ); ?>"><?php echo esc_html( $info['name'] ); ?></option><?php endforeach; ?></select></td></tr>
		<tr><th><label for="guild_rank"><?php esc_html_e( 'Guild Rank', 'gw2-guild-login' ); ?></label></th><td><?php $ranks = GW2_Guild_Ranks::instance()->fetch_guild_data( current( get_option( 'gw2gl_settings' )['guild_ids'] ?: array() ) )['ranks'] ?? array(); ?><select name="guild_rank">
		<?php
		foreach ( $ranks as $rank ) :
			?>
			<option><?php echo esc_html( is_array( $rank ) ? ( $rank['name'] ?: '' ) : esc_html( $rank ) ); ?></option><?php endforeach; ?></select></td></tr>
	</table>
