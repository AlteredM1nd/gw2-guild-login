...
if (isset($_GET['action']) && $_GET['action'] === 'gw2_export_data') {
...
$selected_period = sanitize_text_field($_GET['period'] ?? '7_days');
$start_date = sanitize_text_field($_GET['start_date'] ?? '');
$end_date = sanitize_text_field($_GET['end_date'] ?? '');
...
// Function to get login activity data
function gw2_get_login_activity($start_timestamp, $end_timestamp, $search_user = '') {
    global $wpdb;
    ...
    if ($search_user) {
        ...
        if (!empty($user_ids)) {
            $user_filter = 'AND user_id IN (' . implode(',', array_map('intval', $user_ids)) . ')';
        } else {
    ...
    $results = $wpdb->get_results($wpdb->prepare(
        "SELECT user_id, meta_value as last_login 
         FROM {$wpdb->usermeta} 
         WHERE meta_key = 'gw2_last_login' 
         AND CAST(meta_value AS SIGNED) BETWEEN %d AND %d 
         {$user_filter}
    ...
    foreach ($results as $row) {
        ...
        if ($user) {
            $activity_data[] = [
                ...
                'last_login' => date('Y-m-d H:i:s', intval($row->last_login)),
                'gw2_account' => get_user_meta($row->user_id, 'gw2_account_name', true)
            ];
        }
    }
...
function gw2_get_failed_attempts($start_timestamp, $end_timestamp) {
    global $wpdb;
        ...
        if (is_array($data) && isset($data['count'], $data['time'])) {
            ...
            if ($attempt_time >= $start_timestamp && $attempt_time <= $end_timestamp) {
                ...
                $failed_attempts[] = [
                    ...
                    'last_attempt' => date('Y-m-d H:i:s', $attempt_time)
                ];
            }
        }
    }
...
function gw2_get_user_engagement($search_user = '') {
    global $wpdb;
    
    $user_filter = '';
    if ($search_user) {
        $user_filter = $wpdb->prepare(
    ...
    $engagement_data = [];
    foreach ($results as $row) {
        $last_login = $row->last_login_timestamp ? date('Y-m-d H:i:s', intval($row->last_login_timestamp)) : 'Never';
        $days_since_login = $row->last_login_timestamp ? round((time() - intval($row->last_login_timestamp)) / DAY_IN_SECONDS) : 'N/A';
        ...
        $engagement_data[] = [
            ...
            'gw2_account' => $row->gw2_account_name ?: 'Not linked',
            'last_login' => $last_login,
            'days_since_login' => $days_since_login,
            'has_2fa' => $row->has_2fa ? 'Yes' : 'No',
            'status' => gw2_get_user_activity_status($row->last_login_timestamp)
        ];
    }
...
function gw2_get_user_activity_status($last_login_timestamp) {
    if (!$last_login_timestamp) return 'Never logged in';
    
    $days_ago = (time() - intval($last_login_timestamp)) / DAY_IN_SECONDS;
    
    if ($days_ago <= 7) return 'Active (7 days)';
    if ($days_ago <= 30) return 'Recent (30 days)';
    if ($days_ago <= 90) return 'Inactive (90 days)';
    return 'Dormant (90+ days)';
}

// Function to handle data export
function gw2_handle_data_export($report_type, $start_date, $end_date) {
    $filename = "gw2-guild-login-{$report_type}-" . date('Y-m-d') . '.csv';
    ...
    switch ($report_type) {
        case 'login_activity':
            $start_timestamp = $start_date ? strtotime($start_date) : strtotime('-7 days');
            $end_timestamp = $end_date ? strtotime($end_date . ' 23:59:59') : time();
            $data = gw2_get_login_activity($start_timestamp, $end_timestamp);
            
            fputcsv($output, ['User ID', 'Username', 'Display Name', 'Email', 'Last Login', 'GW2 Account']);
            foreach ($data as $row) {
                fputcsv($output, [
                    $row['user_id'],
                    $row['username'],
        ...
        case 'user_engagement':
            ...
            fputcsv($output, ['User ID', 'Username', 'Display Name', 'Email', 'Registered', 'GW2 Account', 'Last Login', 'Days Since Login', '2FA Enabled', 'Status']);
            foreach ($data as $row) {
                fputcsv($output, [
                    $row['user_id'],
                    $row['username'],
...
$gw2_linked = count(array_filter($user_engagement, function($user) { return $user['gw2_account'] !== 'Not linked'; }));
$twofa_enabled = count(array_filter($user_engagement, function($user) { return $user['has_2fa'] === 'Yes'; }));
$active_users_30d = count(array_filter($user_engagement, function($user) {
    return $user['days_since_login'] !== 'N/A' && $user['days_since_login'] <= 30;
}));
...
<div class="wrap gw2-admin-reports">
        ...
        <form method="get" action="">
            ...
            <div style="display: flex; gap: 15px; align-items: end; flex-wrap: wrap;">
                ...
                <div>
                    ...
                    <a href="<?php echo esc_url(admin_url('admin.php?page=' . ($_GET['page'] ?? ''))); ?>" class="button"><?php esc_html_e('Reset', 'gw2-guild-login'); ?></a>
                </div>
            </div>
        </form>
    </div>
    ...
    <nav class="nav-tab-wrapper">
        ...
        <a href="<?php echo esc_url(add_query_arg('tab', 'login_activity')); ?>" 
           class="nav-tab <?php echo $current_tab === 'login_activity' ? 'nav-tab-active' : ''; ?>">
            <?php esc_html_e('Login Activity', 'gw2-guild-login'); ?>
        </a>
        <a href="<?php echo esc_url(add_query_arg('tab', 'user_engagement')); ?>" 
           class="nav-tab <?php echo $current_tab === 'user_engagement' ? 'nav-tab-active' : ''; ?>">
    ...
    <div class="tab-content" style="margin-top: 20px;">
        ...
        <?php if ($current_tab === 'overview'): ?>
            <!-- Overview Dashboard -->
            <div class="gw2-dashboard-stats" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">                <div class="gw2-stat-card card" style="padding: 20px; text-align: center;">
                    ...
                    <small style="color: #666;"><?php echo esc_html(sprintf(__('in last %s', 'gw2-guild-login'), $selected_period === 'custom' ? 'selected period' : str_replace('_', ' ', $selected_period))); ?></small>
                </div>
        ...
        <?php elseif ($current_tab === 'login_activity'): ?>
            <!-- Login Activity Details -->
            <div class="card" style="padding: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <h3><?php esc_html_e('Login Activity Details', 'gw2-guild-login'); ?></h3>
                    <a href="<?php echo esc_url(add_query_arg(['action' => 'gw2_export_data', 'report_type' => 'login_activity', 'start_date' => $start_date, 'end_date' => $end_date])); ?>" 
                            ...
                            <tbody>
                                <?php foreach ($login_activity as $login): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($login['gw2_account']): ?>
                                                <span style="color: #4CAF50;">✓ <?php echo esc_html($login['gw2_account']); ?></span>
                                            <?php else: ?>
                                                <span style="color: #f44336;">✗ Not linked</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?php echo esc_html($login['last_login']); ?></td>
                                        <td>
                                            <a href="<?php echo esc_url(admin_url('user-edit.php?user_id=' . $login['user_id'])); ?>" 
        ...
        <?php elseif ($current_tab === 'user_engagement'): ?>
            <!-- User Engagement Analysis -->
            <div class="card" style="padding: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <h3><?php esc_html_e('User Engagement Analysis', 'gw2-guild-login'); ?></h3>
                    <a href="<?php echo esc_url(add_query_arg(['action' => 'gw2_export_data', 'report_type' => 'user_engagement'])); ?>" 
                       class="button button-secondary">
                        <?php esc_html_e('Export CSV', 'gw2-guild-login'); ?>
                    </a>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px; padding: 15px; background: #f9f9f9; border-radius: 4px;">
                    <?php
                    $status_counts = array_count_values(array_column($user_engagement, 'status'));
                    // Variables $gw2_linked and $twofa_enabled are already defined globally above
                    ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($user['gw2_account'] !== 'Not linked'): ?>
                                                <span style="color: #4CAF50;"><?php echo esc_html($user['gw2_account']); ?></span>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <?php if ($user['last_login'] !== 'Never'): ?>
                                                <?php echo esc_html($user['last_login']); ?>
                                                <br><small style="color: #666;"><?php echo esc_html($user['days_since_login']); ?> days ago</small>
                                            <?php else: ?>
                                        ...
                                        <td>
                                            <span class="gw2-status-badge" data-status="<?php echo esc_attr(strtolower(str_replace(' ', '-', $user['status']))); ?>">
        ...
        <?php elseif ($current_tab === 'security'): ?>
            ...
            <div class="card" style="padding: 20px; margin-bottom: 20px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin: 20px 0;">                    <div style="padding: 15px; background: #f0f8ff; border-left: 4px solid #2196F3; border-radius: 4px;">
            ...
            <div class="card" style="padding: 20px;">
                ...
                <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($failed_attempts as $attempt): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><strong><?php echo esc_html($attempt['attempts']); ?></strong></td>
                                        ...
                                        <td>
                                            <a href="https://whatismyipaddress.com/ip/<?php echo esc_attr($attempt['ip_address']); ?>" 
                    ...
                    <p style="margin-top: 15px; color: #666;">
                        <?php echo esc_html(sprintf(__('Total: %d failed attempts from %d unique IPs', 'gw2-guild-login'), $total_failed, count($failed_attempts))); ?>
                    </p>
                <?php endif; ?>
            </div>
